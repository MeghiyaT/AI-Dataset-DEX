...
export const NETWORK_CONFIGS: Record<NetworkId, NetworkConfig> = {
  ...
  preprod: {
    ...
    proofServer: 'http://127.0.0.1:6300',
    faucet: 'https://midnight-tmnpreprod.nethermind.dev',
    composeServices: ['proof-server'],
  },
};
...
// passphrase). Lace derives seeds the same way.
...
function cliMain(argv: string[]): number {
  ...
  if (candidate !== 'undeployed') {
    ...
    if (!seed) {
      process.stdout.write(`Wallet not yet generated — run \`npm run setup -- --network ${candidate}\` to fund and deploy.\n`);
    }
  }
  return 0;
}
